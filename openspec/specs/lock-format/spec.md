...
Each action entry in the lock file SHALL be a TOML inline table with six fields: `sha`, `version`, `comment`, `repository`, `ref_type`, and `date`.
...
- **THEN** the lock file entry is:
  ```toml
  "actions/checkout@^6" = { sha = "de0fac2e...", version = "v6.2.3", comment = "v6", repository = "actions/checkout", ref_type = "release", date = "2026-02-15T10:35:00Z" }
  ```
...
- **GIVEN** an action `rust-lang/crates-io-auth-action@~1.0.3` resolved to SHA `b7e9a28e...`
- **THEN** `version = "v1.0.3"` and `comment = "v1.0.3"`
...
- **GIVEN** the ref `^6` resolves to a tag that has an associated GitHub Release
- **THEN** `ref_type = "release"`

#### Scenario: Tag without a GitHub Release
- **GIVEN** the ref `^1` resolves to a tag with no associated GitHub Release
- **THEN** `ref_type = "tag"`
...
### Requirement: Migration from v1.0 to v1.4
The system SHALL transparently migrate v1.0 lock files to v1.4 format.
...
- **THEN** keys are migrated from `@v6` to `@^6` format
- **THEN** the file is rewritten in v1.4 format with inline tables
...
- **THEN** the `comment` field is derived from the key version part (e.g., `@v6` → `comment = "v6"`)
- **THEN** keys are migrated from `@v6` to `@^6` format

### Requirement: Migration from v1.3 to v1.4
The system SHALL transparently migrate v1.3 lock files to v1.4 format.

#### Scenario: v1.3 lock migration
- **GIVEN** a v1.3 lock file with `specifier` field and `@v6` style keys
- **WHEN** the lock file is loaded
- **THEN** keys are migrated from `@v6` to `@^6` format
- **THEN** `specifier` field is dropped, `comment` field is derived from old key version
- **THEN** the parsed lock signals `migrated = true`
...
- **GIVEN** manifest specifier `^4` resolves to SHA `abc123`
...
- **GIVEN** manifest specifier `^1` resolves to SHA `def456`
- **AND** only tag `v1` points to SHA `def456`
- **THEN** the lock entry has `version = "v1"`

#### Scenario: Pre-release version stored as-is
- **GIVEN** manifest specifier `~3.0.0-beta.2` resolves to SHA `789abc`
- **AND** tag `v3.0.0-beta.2` points to SHA `789abc`
- **THEN** the lock entry has `version = "v3.0.0-beta.2"`

### Requirement: `comment` field stores human-readable version
The `comment` field SHALL contain the human-readable v-prefixed version derived from the specifier's `to_comment()` method.

#### Scenario: Major precision comment
- **GIVEN** specifier `^4`
- **THEN** `comment = "v4"`

#### Scenario: Minor precision comment
- **GIVEN** specifier `^4.2`
- **THEN** `comment = "v4.2"`

#### Scenario: Patch precision comment
- **GIVEN** specifier `~4.1.0`
- **THEN** `comment = "v4.1.0"`
