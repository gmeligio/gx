...
2. An action entry exists for `(ActionId, resolution.version)` with all metadata fields populated: `sha` (non-empty), `repository` (non-empty), `ref_type` (set), `date` (non-empty)
...
- **THEN** the spec is complete
- **BECAUSE** the resolution has a non-empty version and the action entry is fully populated
