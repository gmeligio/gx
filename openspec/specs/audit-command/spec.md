...
Users check the actions recorded in `gx.lock` against networked, time-varying knowledge —
security advisories and repository state — using `gx audit`. SHA pinning is what the
ecosystem recommends and what Dependabot and Renovate stop covering; this capability is how
a pinned project learns its pins have gone bad.
