...
- [ ] 3.3 Confirm `src/init/report.rs` and `src/lint/report.rs` need no change — neither constructs a renamed variant
- [ ] 3.4 Build and confirm no remaining `Line { action: ... }` construction sites exist anywhere in `src/` or `tests/`
...
- [ ] 4.4 `src/lint/unsynced_manifest.rs`: drop the leading `action ` from both messages (used-not-declared and declared-not-used)
- [ ] 4.5 Re-grep `src/lint/` for a kind-noun prefix in any remaining message to confirm the four were the only ones

## 5. Guard against reintroduction

- [ ] 5.1 Add `format_line_lint_diag_message_carries_no_noun` to `src/output/lines.rs` tests, directly beside `format_line_lint_diag_renders_location_once` and in the same shape, asserting a `LintDiag` message does not open with a kind-noun (`action `, `workflow `, `component `)
...
- [ ] 5.3 Verify both new tests actually fail when the prefix is temporarily restored, then restore the fix
...
- [ ] 6.4 Review the full diff and confirm the only changed user-visible strings are the four lint messages named in section 4
