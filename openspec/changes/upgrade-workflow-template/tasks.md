...
- [x] 1.2 Add the `jq` steps: one emitting `up_to_date` as a step output, and one — gated on that output being `false` — rendering `upgrades[]` into a Markdown body file (`action: from → to`, linked to `compare` when the key is present, plain when absent). Split so the body is never built for a run where nothing moved.
...
- [x] 1.5 Note in `docs/upgrade-workflow.md` that the body omits `in_range` because the default `gx upgrade` is safe mode — a user who switches to `--latest` loses that signal. Kept out of the template: the rationale serves someone deciding whether to customize, not someone pasting the file.
...
- [x] 2.3 Confirm every field the template reads (`upgrades[].action`, `.from`, `.to`, `.compare`, `workflows_updated`, `up_to_date`) matches `src/upgrade/report.rs`.
