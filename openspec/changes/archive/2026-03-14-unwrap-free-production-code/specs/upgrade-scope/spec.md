<!-- MODIFIED: Replaces openspec/specs/upgrade-scope/spec.md -->
<!-- Change: Pinned upgrade construction uses Scope::Pinned(id, version) instead of Mode::Pinned. -->
...
The `Request` for this case SHALL be constructed infallibly as `Request::new(Mode::Safe, Scope::Single(id))`.
...
The `Request` for this case SHALL be constructed infallibly as `Request::new(Mode::Latest, Scope::Single(id))`.
...
The `Request` for this case SHALL be constructed infallibly as `Request::new(Mode::Safe, Scope::Pinned(id, version))`.

#### Scenario: Pinned upgrade of a single action
- **WHEN** user runs `gx upgrade actions/checkout@v5`
- **THEN** `actions/checkout` is upgraded to exactly `v5`
- **THEN** all other actions in the manifest are untouched
