...
> **Reversed for `.config/mise.lock` on 2026-08-03** by `add-mise-lockfile-ci-check`. The premise above held — no hook was bypassed — but the gap bit anyway through a route this decision did not consider: drift originating in **CI's own install conditions**, not in a contributor's commit. Release-plz run 30849375177 aborted on a dirty `.config/mise.lock` after a cold mise cache made `mise install` write entries a warm cache skips. No hook installation could have prevented that, because nothing about the commit was wrong. A `Lockfile` job now verifies `.config/mise.lock`; `.github/gx.lock` still has no CI check, and that part of this decision stands (its writer is gx compiled from this repo's pinned source, so it cannot rewrite the lockfile independently of a commit).
