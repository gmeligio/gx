...
- [x] 1.4 Add builder methods, each called from at least one test: `with_all_tags`, `with_sha_tags`, `with_fixed_sha`, `with_lookup_result`, `failing_auth`, `failing_action`, `failing_describe`, `with_empty_dates`. **Two planned knobs were deleted as dead:** `with_tags_result` (zero callers) and `failing(error)` (only caller was `failing_auth`, its own wrapper — inlined). Seven knobs remain, every one reached from a test
