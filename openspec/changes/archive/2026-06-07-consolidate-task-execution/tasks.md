...
- [x] 4.1 Stage a deliberately misformatted `.rs` change, `git commit`, and confirm the commit succeeds in one shot with formatting applied and re-staged (D3 zero-friction). Verified: ran the `cargo-fmt` hook against a mangled staged `src/lib.rs` via `prek run cargo-fmt`; the hook formatted and `git add -u` re-staged it (staged blob became correctly formatted). File restored after the test.
- [x] 4.2 Confirm `mise run format:check` and `mise run clippy:check` are non-mutating: clean tree before and after. Verified: tree clean before and after both runs.
- [x] 4.3 Run `mise run check`, `mise run clippy:clippy`, `mise run format:format` locally and confirm success. Verified: all three exit 0, tree stays clean.
- [x] 4.4 Push the branch and confirm all `build.yml` jobs pass (Format, Clippy, Check, Unit/Integration/E2E Tests, Deny). Verified on PR #103: all six check jobs green via `mise run <task>` (release-only jobs skipped as expected).
- [x] 4.5 `AGENTS.md` line 2 is generic (`mise run <task>`, no specific name) so it needs no edit; confirm no other doc, README, or `docs/demo.tape` references the now-removed bare `format`/`clippy` task names (grep already done in §1.7) — update any that do. Verified: no docs reference the bare names.
