...
Today only *one* `VersionRegistry` method is deduplicated (`describe_sha`, via `ShaIndex`), and the method that repeats hardest — `all_tags`, called once per manifest spec inside the upgrade loop — is not deduplicated at all.

## What Changes

- Memoize **every** `VersionRegistry` method for the duration of a single command run: repeat lookups of the same key are served from memory instead of the network.
