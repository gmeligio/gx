...
- **WHEN** they commit
- **THEN** the `cargo-fmt` hook runs `mise run format` (mutating), the result is re-staged with `git add`, and the commit succeeds without being rejected for formatting
