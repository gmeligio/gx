...
- **Add `:check` task variants:** `format:check` (`cargo fmt --all --check`), `clippy:check` (`cargo clippy --locked --tests -- -D warnings`, no `--fix`), and a `check` task (`cargo check --locked`).
...
- **Point pre-commit hooks at their tasks** with re-staging: `cargo-fmt` → `mise run format` + `git add`, `cargo-linter` → `mise run clippy` + `git add`, `gx-lockfile` → `mise run tidy` (fixes a latent stale-installed-binary bug; the hook currently runs the globally-installed `gx`, not the source under commit).
...
- **`.config/mise/tasks/`** — new tasks: `format:check`, `clippy:check`, `check`. The existing `clippy` task's `depends` is reconsidered to avoid double-formatting.
