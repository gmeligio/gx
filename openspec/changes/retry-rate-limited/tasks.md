...
- [ ] 1.3 Add `normalize_reset(reset_epoch: u64, now_epoch: u64) -> RetryAfter` in `src/infra/github/registry.rs`: saturating subtraction, returning `TooDistant` when the result exceeds `MAX_RETRY_WAIT` and `Unstated` when the header is absent or unparseable.
- [ ] 1.4 Unit-test `normalize_reset` directly: near reset → `After`, one-hour reset → `TooDistant`, past reset (now ahead of reset) → `After(0)` not a negative or panic, unparseable → `Unstated`.
