...
- [ ] 1.3 Update `infra::workflow_scan::FileScanner` to parse the full YAML once and emit both `Parsed` and the existing `Located` action list. Verify existing tests still pass.
...
- [ ] 3.3 `src/lint/dangerous_trigger.rs` — error if `Parsed.on` contains `pull_request_target`. Unit tests: pull_request only, pull_request_target only, both.
...
- [ ] 3.6 `src/lint/unprotected_secrets.rs` — error per the design.md algorithm. Unit tests: PR workflow + secret + no gate (errors), PR workflow + secret + correct gate (clean), pull_request_target + secret (clean — `dangerous-trigger` covers this), non-PR workflow + secret (clean).
